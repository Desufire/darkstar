...
function onTrade(player,npc,trade)
	...
	if (CUSTOM_TRIAL_ID == 0) then
		...
		if (trade:hasItemQty(2184,1) and trade:getItemCount() == 1) then
			player:SpoofChatPlayer( "I cannot accept that right now, you do not have an active trial.", MESSAGE_ECHO, npc:getID() );
		---------------------
		-- Relic Trials Start
		---------------------
		-- Amanomurakumo
		...
		if (player:getVar("TRIAL_COMPLETE") >= 1) then -- Only triggers if trial active AND done.
			player:SpoofChatPlayer( "Well done, now trade your item to my friend here for your reward.", MESSAGE_ECHO, npc:getID() );
		...
		elseif (trade:hasItemQty(3925,50) and trade:getItemCount() == 25) then
			player:setVar("TRIAL_COMPLETE",2);
			player:SpoofChatPlayer( "Well done, I see you have brought me the items I requested.", MESSAGE_ECHO, npc:getID() );
			player:SpoofChatPlayer( "Trade them along with the trial item to my friend here for your reward.", MESSAGE_ECHO, npc:getID() );
			player:SpoofChatPlayer( "calls forth the Magian Moogle.", MESSAGE_EMOTION, npc:getID() );
			GetNPCByID(npc:getID()+1):setStatus(STATUS_NORMAL);
			player:injectActionPacket(6, 617);
		...
		elseif (trade:hasItemQty(2184,1) and trade:getItemCount() == 1) then
			...
			player:SpoofChatPlayer( "Ok your trial has been canceled.", MESSAGE_ECHO, npc:getID() );
			player:SpoofChatPlayer( "You can now begin a new trial.", MESSAGE_ECHO, npc:getID() );
			player:tradeComplete();
		else -- Trading anything other than expected during an active trial triggers this.
			-- player:PrintToPlayer("Testing else");
			player:SpoofChatPlayer( "If you wish to cancel your current trial...", MESSAGE_ECHO, npc:getID() );
			player:SpoofChatPlayer( "The fee for wasting my time is:", MESSAGE_ECHO, npc:getID() );
...
function textRelic1(player,npc,trade)
	player:SpoofChatPlayer( " ........ Caladbolg Trials ........ ", MESSAGE_ECHO, npc:getID() );
end;

function textRelic2(player,npc,trade)
	player:injectActionPacket(4, 918);
	player:SpoofChatPlayer( "Objective: Monster slaying.", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Intulo x2", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Megalobugard x1", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Seww the Squidlimbed x1", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Mischievous Micholas x2", MESSAGE_ECHO, npc:getID() );
...
function textRelic3(player,npc,trade)
	player:injectActionPacket(4, 918);
	player:SpoofChatPlayer( "Objective: Monster slaying.", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Fafnir x2", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Adamantoise x2", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Behemoth x2", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "On completion of your trial please ensure you have at least 1 free inventory space.", MESSAGE_ECHO, npc:getID() );
end;
...
function textRelic4(player,npc,trade)
	player:injectActionPacket(4, 918);
	player:SpoofChatPlayer( "Objective: Monster slaying.", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Nidhogg x1", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "King Behemoth x1", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Aspidochelone x1", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "On completion of your trial please ensure you have at least 1 free inventory space.", MESSAGE_ECHO, npc:getID() );
end;

function textRelic5(player,npc,trade)
	player:injectActionPacket(4, 918);
	player:SpoofChatPlayer( "Objective: Item gathering.", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "Bring me 25 Tanzanite Jewels.", MESSAGE_ECHO, npc:getID() );
	player:SpoofChatPlayer( "On completion of your trial please ensure you have at least 1 free inventory space.", MESSAGE_ECHO, npc:getID() );
end;
