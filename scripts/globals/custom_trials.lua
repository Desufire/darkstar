...
function doCustomTrial(mob, killer, isKillShot, isWeaponSkillKill)
	...
	if (utils.inTable(Relic1, TRIAL) == true and (MAIN == TRIAL or SUB == TRIAL or RANGED == TRIAL)) then
		if (SewwTheSquidLimbedKillCount < 1 and mob:getID() == 17498301) then
			killer:setVar("Seww_the_Squidlimbed_KILLS", SewwTheSquidLimbedKillCount + 1);
		end
		if (MischievousCount < 2 and mob:getID() == 17281149) then
			killer:setVar("Mischievous_Micholas_KILLS", MischievousCount + 1);
		end
		if (MegalobugardKillCount < 1 and mob:getID() == 16875741) then
			killer:setVar("Megalobugard_KILLS", MegalobugardKillCount + 1);
		end
		if (IntuloKillCount < 2 and mob:getID() == 16793742) then
		...
		if (MegalobugardKillCount == 1 and SewwTheSquidLimbedKillCount == 1 and MischievousCount == 2 and IntuloKillCount == 2) then
	...
	if (utils.inTable(Relic2, TRIAL) == true and (MAIN == TRIAL or SUB == TRIAL or RANGED == TRIAL)) then
		if (AdamantoiseKillCount < 2 and mob:getID() == 17301537) then
			killer:setVar("Adamantoise_KILLS", AdamantoiseKillCount + 1);
		end
		if (BehemothKillCount < 2 and mob:getID() == 17297440) then
			killer:setVar("Behemoth_KILLS", BehemothKillCount + 1);
		end
		if (FafnirKillCount < 2 and mob:getID() == 17408018) then
		...
		if (AdamantoiseKillCount == 2 and BehemothKillCount == 2 and FafnirKillCount == 2) then
			killer:setVar("TRIAL_COMPLETE", 1);
		end
	end
	...
	if (utils.inTable(Relic3, TRIAL) == true and (MAIN == TRIAL or SUB == TRIAL or RANGED == TRIAL)) then
		if (NidhoggKillCount < 1 and mob:getID() == 17408019) then
			killer:setVar("Nidhogg_KILLS", NidhoggKillCount + 1);
		end
		if (KingBehemothKillCount < 1 and mob:getID() == 17297441) then
			killer:setVar("King_Behemoth_KILLS", KingBehemothKillCount + 1);
		end
		if (AspidocheloneKillCount < 1 and mob:getID() == 17301538) then
			killer:setVar("Aspidochelone_KILLS", AspidocheloneKillCount + 1);
		end
		------------------
		-- Trial complete
		------------------
		if (NidhoggKillCount == 1 and KingBehemothKillCount == 1 and
			AspidocheloneKillCount == 1) then
			killer:setVar("TRIAL_COMPLETE", 1);
		end
	end
