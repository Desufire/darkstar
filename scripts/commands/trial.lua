...
function RelicTrial1(player,npc)
	...
	local kill3 = player:getVar("Seww_the_Squidlimbed_KILLS");
	local kill4 = player:getVar("Mischievous_Micholas_KILLS");

	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Megalobugard" , kill1, "/1)."), MESSAGE_ECHO, nil );
	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Intulo" , kill2, "/2)."), MESSAGE_ECHO, nil );
	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Seww the Squidlimbed" , kill3, "/1)."), MESSAGE_ECHO, nil );
	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Mischievous Micholas" , kill4, "/2)."), MESSAGE_ECHO, nil );
end;

function RelicTrial2(player,npc)
	local kill1 = player:getVar("Adamantoise_KILLS");
	local kill2 = player:getVar("Behemoth_KILLS");
	local kill3 = player:getVar("Fafnir_KILLS");

	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Adamantoise" , kill1, "/2)."), MESSAGE_ECHO, nil );
	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Behemoth" , kill2, "/2)."), MESSAGE_ECHO, nil );
	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Fafnir" , kill3, "/2)."), MESSAGE_ECHO, nil );
end;

function RelicTrial3(player,npc)
	local kill1 = player:getVar("Aspidochelone_KILLS");
	local kill2 = player:getVar("King_Behemoth_KILLS");
	local kill3 = player:getVar("Nidhogg_KILLS");

	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Aspidochelone" , kill1, "/1)."), MESSAGE_ECHO, nil );
	player:SpoofChatPlayer( string.format( "%s %s%s",":////(King Behemoth" , kill2, "/1)."), MESSAGE_ECHO, nil );
	player:SpoofChatPlayer( string.format( "%s %s%s",":////(Nidhogg" , kill3, "/1)."), MESSAGE_ECHO, nil );
end;
